...
def P2468OAS_v4(
    ...
    for i in range(0,DataN):
        ...
        # Alignment metric (full curve, no window yet)
        col = smooth_bak[:, i]
    ...
    # Global center from mean curve (harmonic2 fallback to argmax)
    ...
    def _harmonic_center(c2_val, prev_center):
        ...
        # pick the axis closest to previous center to avoid 180° flips
        def circ_dist(a, b):
            d = abs(a - b)
    ...
    for i in range(DataN):
        ...
        if cmode == "argmax":
            ...
            center_idx = int(np.nanargmax(smooth_bak[lo_li:up_li, i]) + lo_li)  # placeholder fallback
        else:  # auto
            if weak:
                center_idx = _harmonic_center(c2_values[i], prev_center)
            else:
        ...
        if lo_li < up_li and up_li <= DataM:
            seg_disp = smooth_bak[lo_li:up_li, i]
            local_idx = int(np.nanargmax(seg_disp)) + lo_li
            disp_idx = local_idx
            disp_val = seg_disp[local_idx - lo_li]
        else:
    ...
    if plot:
        # constrained_layout + colorbar triggers layout-engine errors on newer Matplotlib;
        # stick to the default engine to keep plots working.
